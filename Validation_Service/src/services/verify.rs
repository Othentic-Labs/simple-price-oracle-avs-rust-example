...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    let _ = verify_presentation(&args.example_type).await?;
    Ok(())
}

pub async fn verify_presentation(example_type: &ExampleType) -> Result<bool, Box<dyn std::error::Error>> {
    // Read the presentation from disk.
    let presentation_path = "example-json.presentation.tlsn" ; //tlsn_examples::get_file_path(example_type, "presentation");
    ...
    let verification_result = presentation.verify(&provider);

if let Ok(PresentationOutput {
    server_name,
    connection_info,
    transcript,
    ..
    ...
    let time = chrono::DateTime::UNIX_EPOCH + Duration::from_secs(connection_info.time);
    let server_name = server_name.unwrap_or_else(|| "Unknown server".into());
    let mut partial_transcript = transcript.unwrap();
    partial_transcript.set_unauthed(b'X');
